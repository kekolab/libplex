...
public class Server implements Parent, WithDetails<ServerDetails> {
	...
	ServerDetails serverDetails = client.target(getUri())
		.request()
		.get(ServerDetails.class);
	serverDetails.getDirectories()
		.forEach(directory -> directory.setClient(this.client)
			.setServer(this)
			.setParent(this));
